...
// Construye un bloque de leyenda simple (swatch + etiqueta) para una capa
// no-coroplética activa, a partir de su cfg.legendItems (color, borde,
// patrón opcional, etiqueta). Reutiliza legendSwatchStyle (definida más
// abajo para la leyenda de transición) para que un mismo "look" de
// swatch sirva para toda la app.
function buildGeneralLegendBlock(cfg, cardClass) {
  const rows = cfg.legendItems.map((info) => `
    <div class="legend-row">
      <span class="legend-swatch" style="${legendSwatchStyle(info)}"></span>
      <span class="legend-range">${info.label}</span>
    </div>
  `).join("");
  return `
    <div class="${cardClass}">
...
function updateSidebarLegend() {
  ...
  const activeChoros = layersConfig.filter((c) => c.choropleth).filter(isChecked);
  const activeGeneral = layersConfig
    .filter((c) => !c.choropleth && c.legendItems && c.legendItems.length)
    .filter(isChecked);

  if (!activeChoros.length && !activeGeneral.length) {
    const emptyMsg = "<p>Activa una capa para ver su leyenda.</p>";
    if (sidebarEl) sidebarEl.innerHTML = emptyMsg;
    if (mapEl) mapEl.innerHTML = emptyMsg;
    return;
  }

  const generalBlocksForSidebar = activeGeneral.map((cfg) => buildGeneralLegendBlock(cfg, "legend-block"));
  const generalBlocksForMap = activeGeneral.map((cfg) => buildGeneralLegendBlock(cfg, "legend-card"));
  ...
  const htmlSidebar = generalBlocksForSidebar.join("\n") + blocksForSidebar.join("\n");
  const htmlMap = `<div class="map-legend-wrap">${generalBlocksForMap.join("\n")}${blocksForMap.join("\n")}</div>`;
  if (sidebarEl) sidebarEl.innerHTML = htmlSidebar;
  if (mapEl) mapEl.innerHTML = htmlMap;
}


// Colores del "Grupo C. Sistemas cacaoteros destacados" — espejo exacto de
// 02_1_Cobertura_suelo.pdf. cacao_sys -> grupo_c se resuelve en
// convertir_segundo_envio.py (ver ese script para el detalle del mapeo).
const COBERTURA_CACAO_STYLE = {
  "Cultivos de exclusivos de cacao": { fill: "#A57159", border: "#7A4F3D", label: "Cultivos de exclusivos de cacao" },
  "Cultivos mixtos de cacao": { fill: "#F0BB5B", border: "#C28F2E", label: "Cultivos mixtos de cacao" },
  "Cacao con teca y cultivos mixtos": { fill: "#A48A57", border: "#3DBFA0", label: "Cacao con teca y cultivos mixtos" },
  _default: { fill: "#A48A57", border: "#6B7280", label: "Sin clasificar" },
};

// Categorías de la capa "Poblados", por campo Pob_estudi.
const POBLADOS_STYLE = {
  SI: { fill: "#DC2626", border: "#7F1D1D", label: "Poblado de estudio" },
  OTRO: { fill: "#2563EB", border: "#1E3A8A", label: "Poblado (otro)" },
};
...
const layersConfig = [
  ...
  // Estilo según 01_3_Accesibilidad.pdf: tramado diagonal gris (#9CA3AF).
  {
    ...
    pane: "pane_limites",
    legendItems: [{ fill: "#FFFFFF", border: "#9CA3AF", pattern: "hatch", label: "Límite Parroquial" }],
    style: {
      color: "#9CA3AF",
      weight: 1.5,
      fillColor: "url(#hatchParroquial)",
      fillOpacity: 1,
    },
  ...
  // ----- Vías principales -----
  {
    ...
    pane: "pane_tematica",
    legendItems: [{ fill: "#FFFFFF", border: "#111827", label: "Vías principales" }],
    style: (f) => {
  ...
  // ----- Poblados (contexto, desde 2_4_Transicion) -----
  // Se mantienen visibles los 48 poblados (no solo los 3 con
  // Pob_estudi='SI' que filtra el catálogo para el mapa impreso) para no
  // perder cobertura en el visor interactivo; se colorean por categoría
  // según el campo Pob_estudi ("SI" = poblado de estudio, resaltado en
  // rojo; el resto en azul) para distinguirlos a simple vista.
  {
    id: "PobladosTransicion",
    label: "Poblados",
    url: "Poblados_Opt.geojson",
    pane: "pane_puntos",
    legendItems: Object.values(POBLADOS_STYLE),
    pointToLayer: (f, latlng) => {
      const esEstudio = (f?.properties?.Pob_estudi ?? "").toString().toUpperCase() === "SI";
      const info = esEstudio ? POBLADOS_STYLE.SI : POBLADOS_STYLE.OTRO;
      return L.circleMarker(latlng, {
        radius: esEstudio ? 6 : 4,
        color: info.border,
        weight: esEstudio ? 2 : 1,
        fillColor: info.fill,
        fillOpacity: esEstudio ? 0.9 : 0.7,
      });
    },
    onEachFeature: (f, l) => {
      const p = f.properties || {};
  ...
  // una librería adicional).
  {
    ...
    onEachFeature: (f, l, group) => {
      ...
      // Etiqueta con el nombre del área protegida (mismo patrón que Molleturo/Provincial)
      if (nombre !== "s/i" && l.getBounds) {
        const c = l.getBounds().getCenter();
        L.marker(c, {
          icon: L.divIcon({
            className: "label-text",
  ...
  // ----- Red vial estatal (2do envío, via_l, recortada al área de estudio) -----
  // Color según 01_3_Accesibilidad.pdf: línea granate/maroon.
  {
    id: "RedVialEstatal",
    label: "Red vial estatal",
    url: "Red_Vial_Estatal_Opt.geojson",
    pane: "pane_tematica",
    legendItems: [{ fill: "#FFFFFF", border: "#A41C4A", label: "Red vial estatal" }],
    style: {
      color: "#A41C4A",
      weight: 1.6,
      opacity: 0.9,
  ...
  // No tiene color propio en ninguno de los 2 PDF (no aparece en su
  // leyenda); se usa un punto gris neutro para no confundirlo con
  // "Comunidades / Puntos de estudio" (verde) ni "Poblados" (rojo/azul,
  // del catálogo de transición).
  {
    id: "PobladosIGM250",
    label: "Poblados IGM (1:250.000)",
    url: "Poblados_IGM250_Opt.geojson",
    pane: "pane_puntos",
    legendItems: [{ fill: "#94A3B8", border: "#475569", label: "Poblados IGM (1:250.000)" }],
    pointToLayer: (f, latlng) =>
      L.circleMarker(latlng, {
        radius: 3.5,
        color: "#475569",
  ...
  // ----- Cobertura del suelo: sistemas cacaoteros 2015 (2do envío, cacao_2015) -----
  // Colores del "Grupo C. Sistemas cacaoteros destacados" de
  // 02_1_Cobertura_suelo.pdf. ADVERTENCIA: este shapefile solo contiene
  // las categorías de cacao (Grupo C); los grupos A (Bosque, Vegetación
  // secundaria, Agua, Sin información), B (Plátano/Banano, Otros cultivos)
  // y D (Pasto/Ganadería, Plantación forestal, etc.) del mismo mapa no
  // vienen como vector en este envío, así que no se dibujan (no se
  // inventan polígonos que no existen en los datos entregados).
  {
    id: "CoberturaCacao2015",
    label: "Cobertura: sistemas cacaoteros (2015)",
    url: "Cacao_2015_Opt.geojson",
    pane: "pane_tematica",
    legendItems: [
      COBERTURA_CACAO_STYLE["Cultivos de exclusivos de cacao"],
      COBERTURA_CACAO_STYLE["Cultivos mixtos de cacao"],
      COBERTURA_CACAO_STYLE["Cacao con teca y cultivos mixtos"],
    ],
    style: (f) => {
      const info = COBERTURA_CACAO_STYLE[f?.properties?.grupo_c] || COBERTURA_CACAO_STYLE._default;
      return { color: info.border, weight: 0.6, fillColor: info.fill, fillOpacity: 0.85 };
    },
    onEachFeature: (f, l) => {
      const p = f.properties || {};
      l.bindPopup(`
        <b>Categoría:</b> ${p.grupo_c ?? "s/i"}<br>
        <b>Etiqueta:</b> ${p.label ?? "s/i"}<br>
...
// Patrones SVG (tramado) para "Límite Parroquial" y "SNAP", según
// 01_3_Accesibilidad.pdf. Los fill="url(#id)" son reactivos: si el patrón
// se registra después de pintar el path, el navegador lo aplica igual.
if (cfg.pane === "pane_limites") ensureLimitesPatterns();
...
/* =========================================================
   ARRANQUE AUTO
========================================================= */
const autoOnIds = ["Molleturo", "Comunidades"];
...
// Patrones para pane_limites, según la leyenda de 01_3_Accesibilidad.pdf:
// "Límite parroquial de Molleturo" (tramado diagonal gris) y "Sistema
// Nacional de Áreas Protegidas" (tramado verde, aproximado con puntos).
function ensureLimitesPatterns() {
  ensurePanePatterns("pane_limites", [
    { id: "hatchParroquial", bg: "#FFFFFF", line: "#9CA3AF", lineWidth: "1.4" },
