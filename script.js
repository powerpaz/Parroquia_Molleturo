...
// Grupo C (sistemas cacaoteros). Por eso la capa vectorial "Sistemas
// cacaoteros (año 2015)" solo dibuja Grupo C: no se inventan polígonos que
// no existen en los datos entregados. Para los grupos A, B y D se ofrece
// la capa complementaria "Cobertura2015Raster" (imagen recortada y
// georreferenciada del propio mapa oficial 02_1_Cobertura_suelo.pdf, ver
// más abajo en layersConfig) en vez de fabricar geometría. La leyenda
// siempre muestra los 4 grupos completos, tal como exige el catálogo.
...
const layersConfig = [
  ...
  // (2do envío). Ver advertencia sobre cob_2015 junto a COBERTURA_2015_GROUPS.
  {
    ...
    pane: "pane_tematica",
    // Los polígonos de cacao cubren una zona pequeña y específica: sin esto,
    // si el usuario está viendo otra parte del mapa al marcar el checkbox,
    // parece que "no pasó nada" aunque la capa sí se activó.
    fitOnEnable: true,
  ...
  // No existe un shapefile/GeoJSON con los Grupos A, B y D completos (ver
  // advertencia junto a COBERTURA_2015_GROUPS): el dato vectorial solo
  // cubre cacao (Grupo C). En vez de inventar polígonos para los grupos
  // faltantes, se recorta y georreferencia la imagen del mapa OFICIAL
  // "02_1_Cobertura_suelo.pdf" (la columna de leyenda impresa se excluye
  // del recorte; ya tenemos nuestra propia leyenda interactiva). El
  // recuadro magenta "Área de estudio" del PDF es un rectángulo perfecto
  // que coincide con Area_Estudio_Opt.geojson, así que sus 4 esquinas dan
  // las coordenadas reales para anclar la imagen sobre el mapa.
  {
  ...
  if (cfg.id === "CoberturaCacao2015") {
    ...
    COBERTURA_2015_GROUPS.forEach((group, idx) => {
      ...
      if (hasData) {
        ...
        row.title = "Sin polígonos propios: clic aquí para activar 'Mapa oficial cobertura 2015 (imagen)'";
        const note = document.createElement("span");
        note.className = "cobertura2015-note";
        note.textContent = "(clic: ver en imagen oficial)";
        row.appendChild(rowChk);
        row.appendChild(rowLab);
        row.appendChild(note);
        groupsWrap.appendChild(row);

        // Estas 3 filas no tienen checkbox propio (no hay datos vectoriales),
        // así que un clic en cualquier parte de la fila activa directamente
        // la capa de imagen oficial: evita que el usuario tenga que ubicar
        // por su cuenta la fila "Mapa oficial cobertura 2015 (imagen)".
        row.addEventListener("click", () => {
          const rasterChk = document.getElementById("chk_Cobertura2015Raster");
          if (rasterChk && !rasterChk.checked) {
            rasterChk.checked = true;
            rasterChk.dispatchEvent(new Event("change", { bubbles: true }));
          }
        });
        return;
      }
