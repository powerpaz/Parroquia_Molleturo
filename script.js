...
// Arrastrar la leyenda flotante con el mouse (estilo carrusel): las
// tarjetas activas se acomodan en una sola fila con scroll horizontal
// (.map-legend-wrap, ver style.css) en vez de apilarse y tapar el mapa.
// Delegado sobre #mapLegend (estable) en vez de .map-legend-wrap (se
// recrea en cada updateSidebarLegend), así sigue funcionando tras cada
// encendido/apagado de capa.
(() => {
  ...
  legendEl.addEventListener("mousedown", (e) => {
    const wrap = e.target.closest(".map-legend-wrap");
    if (!wrap) return;
    dragging = true;
  ...
  window.addEventListener("mousemove", (e) => {
    if (!dragging) return;
    const wrap = legendEl.querySelector(".map-legend-wrap");
    if (!wrap) return;
