...
// (como en tus referencias de WhatsApp: cajas pequeñas al pie del mapa)
...
const layersConfig = [
  ...
  // ----- Sectores 2022 (coropleta, optimizada) -----
  {
    ...
    // Reemplazo: nueva capa enviada por ti (Sector Censal 2022)
    url: "Sector_Censal_2022_Optimizado.geojson",
    pane: "pane_tematica",
    choropleth: true,
    field: "Num",
    legendStep: 1,
    ...
    onEachFeature: (f, l) => {
      ...
      // En la visualización lo tratamos como "Población 2022" (según tu simbología)
      const anio = "2022";
      l.bindPopup(
  ...
  // ----- Sectores 2010 (coropleta, optimizada) -----
  {
    ...
    // Reemplazo: nueva capa enviada por ti (Sector Censal 2010)
    url: "Sector_Censal_2010_Optimizado.geojson",
    pane: "pane_tematica",
    choropleth: true,
    field: "Pob_total",
    legendStep: 1,
...
layerListEl.addEventListener("change", async (e) => {
  ...
  if (e.target.checked) {
    // UX: indicador de carga y bloqueo temporal del checkbox
    const needsFetch = !cfg._data;
    if (item && needsFetch) item.classList.add("is-loading");
    if (needsFetch) e.target.disabled = true;

    const data = cfg._data || await loadLayerData(cfg);
    cfg._data = data;

    if (needsFetch) e.target.disabled = false;
    ...
    if (!chkNow || !chkNow.checked || token !== cfg._toggleToken) {
      // Si se apagó mientras cargaba, limpiamos estados visuales
      if (needsFetch) e.target.disabled = false;
      if (item) item.classList.remove("is-loading");
      updateSidebarLegend();
      return;
...
/* =========================================================
   ARRANQUE AUTO
========================================================= */
// Capas que se encienden al cargar (como en tu ejemplo)
const autoOnIds = [
  "Molleturo",
  "Comunidades",
  "PuntosCampo",
