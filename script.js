...
// === Responsiveness: ensure Leaflet resizes correctly ===
window.addEventListener('resize', () => { map.invalidateSize({animate:false}); });
window.addEventListener('orientationchange', () => { setTimeout(() => map.invalidateSize({animate:false}), 150); });
document.addEventListener('visibilitychange', () => { if (!document.hidden) setTimeout(() => map.invalidateSize({animate:false}), 100); });
