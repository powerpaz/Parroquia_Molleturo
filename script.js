...
const map = L.map('map', {
  ...
  minZoom: 3,
  maxZoom: 19
}).setView([-2.88, -79.33], 11);
  // === Escala métrica responsive ===
  function calcScaleWidth() {
    const w = map.getSize().x;
    return Math.max(90, Math.min(260, Math.round(w * 0.22)));
  }
  function pickScalePosition() {
    return (window.innerWidth <= 480) ? 'bottomright' : 'bottomleft';
  }
  var __scaleCtl;
  function mountScale() {
    if (__scaleCtl) { try { map.removeControl(__scaleCtl); } catch(e){} }
    __scaleCtl = }
  mountScale();
  map.on('resize', mountScale);
  map.on('zoomend', function(){ setTimeout(mountScale, 0); });
  window.addEventListener('orientationchange', function(){ setTimeout(mountScale, 100); });
 // Molleturo aprox.
