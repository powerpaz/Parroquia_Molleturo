...
// Lo que está arriba en el panel queda encima en el mapa: se recorre de
// abajo hacia arriba aplicando bringToFront en cascada.
function applyLayerDrawOrder() {
  const ids = [...layerListEl.querySelectorAll(".layer-item")].map((el) => el.dataset.layerId);

  ids.slice().reverse().forEach((id) => {
    const group = layerStore.get(id);
    if (!group) return;
    if (group.eachLayer) {
...
// Patrones SVG (tramado) para "SNAP", según 01_3_Accesibilidad.pdf. Los
// fill="url(#id)" son reactivos: si el patrón se registra después de
// pintar el path, el navegador lo aplica igual.
...
// "Sistema Nacional de Áreas Protegidas" (tramado verde, aproximado con
// puntos). El límite parroquial ya no usa tramado (ajuste de última
// revisión: queda sin relleno, solo borde).
