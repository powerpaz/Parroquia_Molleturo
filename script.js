...
const layersConfig = [
  ...
  // =========================================================
  {
    ...
    popup: p => `
      ...
      <b>Nombre:</b> ${p.nam ?? p.Nombre ?? 's/i'}<br>
      <b>Descripción:</b> ${p.descripcio ?? p.Descripcion ?? 's/i'}<br>
    ...
    try {
      ...
      const layer = L.geoJSON(data, {
        ...
        onEachFeature: (feat, lyr) => {
          if (cfg.popup) lyr.bindPopup(cfg.popup(feat.properties));
        }
      });
      ...
      console.error("Error capa:", id, err);
      alert("Error cargando " + id);
    }
...
(async () => {
  ...
  if (parroquia) {
    setTimeout(() =>
      map.fitBounds(parroquia.getBounds(), { padding: [50, 50] }), 400
    );
  }
})();
