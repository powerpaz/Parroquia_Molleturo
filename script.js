...
// Paleta determinista por palabra clave: dos capas distintas que comparten
// la misma categoría (p. ej. "PERSISTENCIA_BOSQUE" en 1990-2000 y en
// 2000-2020) siempre se ven con el mismo color, sin depender del orden
// de aparición.
function getTransitionColor(category) {
  const norm = normalizeTransitionValue(category);
  const isPersistencia = norm.includes("PERSISTENCIA");

  // "Persistencia" (sin cambio de cobertura) se pinta siempre en tonos
  // neutros/grises, y "cambio real" (expansión, pérdida, ganancia,
  // reconfiguración) en colores vivos. Esto evita que, por ejemplo,
  // "EXPANSIÓN_AGRO" y "PERSISTENCIA_USO_AGROPECUARIA" comparen el mismo
  // color solo por compartir la palabra "AGRO": semánticamente una es
  // cambio y la otra es estabilidad, y deben distinguirse a simple vista.
  if (isPersistencia) {
    if (norm.includes("BOSQUE")) return "#166534";
    if (norm.includes("VEGETACION")) return "#4ade80";
    if (norm.includes("AGUA")) return "#0284c7";
    if (norm.includes("URBAN") || norm.includes("INFRAESTRUCTURA")) return "#57534e";
    return "#a8a29e"; // persistencia genérica (agro, pasto, plantación, etc.)
  }

  if (norm.includes("SIN_INFORMACION") || norm.includes("SININFORMACION")) return "#d1d5db";
  if (norm.includes("GANANCIA") && norm.includes("BOSQUE")) return "#16a34a";
  if (norm.includes("PERDIDA") && norm.includes("BOSQUE")) return "#dc2626";
  if (norm.includes("RECONFIGURACION")) return "#7c3aed";

  if (norm.includes("EXPANSION")) {
    if (norm.includes("URBAN")) return "#1f2937";
    if (norm.includes("PLANTACION")) return "#eab308";
    if (norm.includes("PASTO")) return "#f59e0b";
    if (norm.includes("AGRO")) return "#f97316";
    return "#fb923c"; // expansión genérica
  }

  // Categoría no anticipada: asignar color estable de la paleta genérica
  // del manifiesto, en orden de primera aparición.
  const palette = transitionState.manifest?.categoryPalette || ["#64748b"];
  if (!transitionState.categoryColorMap.has(norm)) {
    const idx = transitionState.categoryColorMap.size;
    transitionState.categoryColorMap.set(norm, palette[idx % palette.length]);
  }
  return transitionState.categoryColorMap.get(norm);
}
...
function createTransitionLeafletLayer(layerCfg, data) {
  ...
  const geo = L.geoJSON(data, {
    ...
    style: (feature) => {
      const category = categoryField ? feature.properties?.[categoryField] : layerCfg.label;
      const fill = getTransitionColor(category);
      return {
        ...defaultStyle,
        color: fill,
        fillColor: fill,
...
function updateTransitionLegend() {
  ...
  transitionState.leafletLayers.forEach((_, id) => {
    ...
    data.features.forEach((ft) => {
      const val = field ? ft.properties?.[field] : cfg.label;
      const label = String(val ?? "Sin información").trim();
      categories.set(label, getTransitionColor(label));
    });
  });

  const rows = Array.from(categories.entries()).slice(0, 20).map(([cat, color]) => `
    <div class="legend-row">
      <span class="legend-swatch" style="background:${color}"></span>
      <span class="legend-range">${htmlEscape(cat)}</span>
    </div>
  `).join("");
