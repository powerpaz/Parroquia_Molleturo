...
// Leyenda flotante (abajo en el mapa): capas "simples" (un solo color o
// pocos ítems) en fila horizontal con scroll.
...
// Leyenda flotante lateral (franja vacía a la derecha del mapa): capas con
// leyenda larga/agrupada (p.ej. "Cobertura y uso del suelo (año 2015)",
// con sus 4 grupos A-D) se separan aquí en vez de inflar la fila de abajo.
const mapLegendRight = L.control({ position: "topright" });
mapLegendRight.onAdd = function () {
  const div = L.DomUtil.create("div", "map-legend-right");
  div.id = "mapLegendRight";
  L.DomEvent.disableClickPropagation(div);
  L.DomEvent.disableScrollPropagation(div);
  return div;
};
mapLegendRight.addTo(map);

// Arrastrar las leyendas flotantes con el mouse (estilo carrusel): las
// tarjetas activas se acomodan en fila (abajo, scroll horizontal) o en
// columna (derecha, scroll vertical) en vez de apilarse y tapar el mapa.
// Delegado sobre los contenedores estables (#mapLegend / #mapLegendRight,
// no .map-legend-wrap[-vertical], que se recrean en cada
// updateSidebarLegend), así sigue funcionando tras cada encendido/apagado
// de capa.
function enableLegendDrag(containerId, wrapSelector, axis) {
  ...
  containerEl.addEventListener("mousedown", (e) => {
    ...
    startScroll = axis === "x" ? wrap.scrollLeft : wrap.scrollTop;
    e.preventDefault();
  });

  window.addEventListener("mousemove", (e) => {
    if (!dragging) return;
    const wrap = containerEl.querySelector(wrapSelector);
    if (!wrap) return;
    const pos = axis === "x" ? e.pageX : e.pageY;
    if (axis === "x") wrap.scrollLeft = startScroll - (pos - start);
    else wrap.scrollTop = startScroll - (pos - start);
  });
...
enableLegendDrag("mapLegend", ".map-legend-wrap", "x");
enableLegendDrag("mapLegendRight", ".map-legend-wrap-vertical", "y");
...
// Capas cuya leyenda es larga/agrupada (varias secciones A-D, etc.): se
// muestran en el panel lateral derecho en vez de la fila horizontal de
// abajo, para no inflarla con una sola tarjeta gigante.
const LEGEND_RIGHT_PANEL_IDS = new Set(["CoberturaCacao2015"]);

function updateSidebarLegend() {
  const sidebarEl = document.getElementById("legend");
  const mapEl = document.getElementById("mapLegend");
  const mapRightEl = document.getElementById("mapLegendRight");
  if (!sidebarEl && !mapEl && !mapRightEl) return;
  ...
  const activeBottom = activeGeneral.filter((cfg) => !LEGEND_RIGHT_PANEL_IDS.has(cfg.id));
  const activeRight = activeGeneral.filter((cfg) => LEGEND_RIGHT_PANEL_IDS.has(cfg.id));

  if (mapEl) {
    if (!activeBottom.length && !activeChoros.length) {
      mapEl.innerHTML = "";
    } else {
      const mapBlocks = activeBottom.map((cfg) => buildGeneralLegendBlock(cfg, "legend-card")).join("\n")
  ...
  if (mapRightEl) {
    if (!activeRight.length) {
      mapRightEl.innerHTML = "";
    } else {
      const rightBlocks = activeRight.map((cfg) => buildGeneralLegendBlock(cfg, "legend-card")).join("\n");
      mapRightEl.innerHTML = `<div class="map-legend-wrap-vertical">${rightBlocks}</div>`;
    }
  }
}
...
const POBLADOS_IGM250_STYLE = {
  "CABECERA CANTONAL": { fill: "#FF0000", border: "#FF0000", radius: 7, fillOpacity: 1, label: "Municipal capital" },
  "PARROQUIA RURAL": { fill: "#FF73DF", border: "#FF73DF", radius: 7, fillOpacity: 1, label: "Rural parish head" },
  "PARROQUIA URBANA": { fill: "#A7C636", border: "#A7C636", radius: 7, fillOpacity: 1, label: "Urban parish head" },
  POBLADO: { fill: "transparent", border: "#000000", radius: 6, fillOpacity: 0, label: "Settlement" },
  _default: { fill: "transparent", border: "#000000", radius: 6, fillOpacity: 0, label: "Settlement" },
};
...
// Estado de la superficie de rodadura (estado_sup) del Inventario Vial
// GADPA (5to envío): semáforo típico de inventarios viales (bueno/regular/
// malo), igual convención que POBLADOS_IGM250_STYLE (color por categoría).
const INVENTARIO_VIAL_STYLE = {
  BUENO: { color: "#2E7D32", label: "Good" },
  REGULAR: { color: "#F5A623", label: "Fair" },
  MALO: { color: "#D32F2F", label: "Poor" },
  _default: { color: "#6B7280", label: "Not surveyed" },
};
...
// Desde el 4to envío ya se cuenta con el shapefile completo cob_2015.shp
// (convertido a Cobertura_2015_Opt.geojson), que cubre los 4 grupos A-D;
// la capa vectorial "Cobertura y uso del suelo (año 2015)" los dibuja todos.
const COBERTURA_2015_GROUPS = [
  {
    title: "Group A. Natural and Environmental Land Cover",
    items: [
      { key: "Bosque", fill: "#267300", label: "Forest" },
      { key: "Vegetación secundaria", fill: "#BFD999", label: "Secondary Vegetation" },
      { key: "Agua y elementos naturales", fill: "#0070FF", label: "Water and Unvegetated Natural Features" },
      { key: "Sin información", fill: "#828282", label: "No Data" },
    ],
  },
  {
    title: "Group B. Food Crops",
    items: [
      { key: "Plátano / Banano", fill: "#E4EDC2", label: "Plantain / Banana" },
      { key: "Otros cultivos alimentarios", fill: "#FFDE3E", label: "Other Food Crops" },
    ],
  },
  {
    title: "Group C. Cocoa Production Systems",
    items: [
      { key: "Cultivos exclusivos de cacao", fill: "#732600", label: "Cocoa-Only Cropping Systems" },
      { key: "Cultivos mixtos de cacao", fill: "#E69800", label: "Mixed Cocoa Cropping Systems" },
      { key: "Cacao con teca y cultivos mixtos", fill: "#734C00", border: "#00FFC5", weight: 1, label: "Cocoa with Teak and Mixed Crops" },
    ],
  },
  {
    title: "Group D. Other Productive and Anthropogenic Uses",
    items: [
      { key: "Sistema silvopastoril", fill: "#70A800", label: "Silvopastoral System" },
      { key: "Pasto / Ganadería", fill: "#AAFF00", label: "Pasture / Livestock" },
      { key: "Plantación forestal", fill: "#3CAF99", label: "Forest Plantation" },
      { key: "Tierras despejadas / degradadas", fill: "#FFA77F", label: "Cleared / Degraded Land" },
      { key: "Otros usos productivos", fill: "#E8BEFF", label: "Other Productive Uses" },
      { key: "Infraestructura y áreas pobladas", fill: "#4E4E4E", label: "Infrastructure and Settlements" },
    ],
  },
];
...
// El shapefile entregado (cob_2015.shp) describe algunas categorías con
// frases largas en el campo "cob_label" (no con el nombre corto del
// catálogo); se traducen aquí a la clave canónica para reutilizar la misma
// paleta sin duplicar colores.
const COB_LABEL_RAW_TO_KEY = {
  ...
  [normalizeUpperNoAccents("Agua y elementos naturales sin vegetación")]: "Agua y elementos naturales",
};
...
const layersConfig = [
  ...
  // Estilo según 01_3_Accesibilidad.pdf: tramado diagonal gris (#9CA3AF).
  {
    ...
    onEachFeature: (f, l, group) => {
      ...
      l.bindPopup(`<b>Parish:</b> ${nombre}<br><b>Canton:</b> ${p.DPA_DESCAN ?? "N/A"}<br><b>Province:</b> ${p.DPA_DESPRO ?? "N/A"}`);
  ...
  {
    id: "LimiteProvincial",
    label: "Provincial Boundary (National Government)",
    url: "Provincial.topo.json",
    type: "topo_polygons",
    topoObject: "Provincial",
    pane: "pane_limites",
    legendItems: [{ fill: "#FFFFFF", border: "#000000", label: "Provincial Boundary (National Government)" }],
    style: {
      color: "#000000",
      weight: 1.5,
      fillOpacity: 0,
    ...
    onEachFeature: (f, l, group) => {
      ...
      l.bindPopup(`<b>Province:</b> ${nombre}`);
  ...
  // "Límite provincial (GAD Provincial de Azuay)" = línea olivo punteada.
  {
    ...
    label: "Provincial Boundary (GAD Azuay)",
    url: "Limite_Provincial_GAD_Opt.geojson",
    pane: "pane_limites",
    legendItems: [{ fill: "#FFFFFF", border: "#979F26", label: "Provincial Boundary (GAD Azuay)" }],
    style: {
      color: "#979F26",
      weight: 2,
      dashArray: "8 5",
    ...
    onEachFeature: (f, l) => {
      const p = f.properties || {};
      l.bindPopup(`<b>Province (GAD):</b> ${p.Provincia ?? "Azuay"}`);
    },
  },
  ...
  {
    id: "Comunidades",
    label: "Study Area Settlements",
    url: "Puntos_de_Estudio.geojson",
    pane: "pane_puntos",
    filter: (f) => !isExcludedPlaceName(f),
    legendItems: [{ fill: "#38AA00", border: "#1F6B00", label: "Study Area Settlements" }],
    ...
    onEachFeature: (f, l, group) => {
      const p = f.properties || {};
      const nombre = p.Nombre ?? p.nam ?? "No name";
      const pob = p.Pob_estudi ?? "N/A";

      // Popup
      l.bindPopup(`
        <b>Community:</b> ${nombre}<br>
        <b>Study population:</b> ${pob}
      `);
  ...
  {
    id: "ViasPrincipales",
    label: "State transportation network",
    url: "Vias_Principales_Optimizado.geojson",
    pane: "pane_tematica",
    legendItems: [{ fill: "#FFFFFF", border: "#111827", label: "State transportation network" }],
    style: (f) => {
    ...
    onEachFeature: (f, l) => {
      ...
      const hwy = p.highway ?? "N/A";
      const clase = p.CLASE_VIA ?? "N/A";
      l.bindPopup(`
        <b>Road:</b> ${name}<br>
        <b>Highway:</b> ${hwy}<br>
        <b>Class:</b> ${clase}
  ...
  {
    id: "Sectores2022",
    label: "Population (No. of Inhabitants) 2022",
    ...
    field: "Num",
    legendStep: 1,
    legendTitle: "Population (No. of Inhabitants) 2022",
    fitOnEnable: true,
    ...
    onEachFeature: (f, l) => {
      const p = f.properties || {};
      const total = p.Num ?? "N/A";
      l.bindPopup(`<b>Population 2022:</b> ${total}`);
    },
  },
  ...
  {
    id: "Sectores2010",
    label: "Population (No. of Inhabitants) 2010",
    ...
    field: "Pob_total",
    legendStep: 1,
    legendTitle: "Population (No. of Inhabitants) 2010",
    fitOnEnable: true,
    // Discretización: cortes fijos (como tu captura)
    classBreaks: {
      min: 6,
    ...
    onEachFeature: (f, l) => {
      const p = f.properties || {};
      const pob = p.Pob_total ?? "N/A";
      l.bindPopup(`<b>Population 2010:</b> ${pob}`);
    },
  },
  ...
  // "Área de estudio" = línea magenta sólida.
  {
    ...
    pane: "pane_limites",
    legendItems: [{ fill: "#FFFFFF", border: "#BD09E8", label: "Study Area" }],
    style: {
      color: "#BD09E8",
      weight: 2.5,
      fillOpacity: 0,
    ...
    onEachFeature: (f, l) => {
      const p = f.properties || {};
      l.bindPopup(`<b>Study area:</b> ${p.Name ?? "Molleturo"}`);
    },
  },
  ...
  {
    id: "RioPrincipal",
    label: "Hydrographic network (study area)",
    url: "Rio_Principal_Opt.geojson",
    pane: "pane_tematica",
    legendItems: [{ fill: "#FFFFFF", border: "#005CE6", label: "Hydrographic network (study area)" }],
    style: (f) => {
    ...
    onEachFeature: (f, l) => {
      const p = f.properties || {};
      l.bindPopup(`
        <b>River:</b> ${p.nam ?? "N/A"}<br>
        <b>Regime:</b> ${p.hyp_desc ?? "N/A"}
      `);
    },
  },
  ...
  {
    id: "SNAP",
    label: "National System of Protected Areas",
    url: "SNAP_Opt.geojson",
    pane: "pane_limites",
    filter: (f) => SNAP_ALLOWED_NAMES.has(normalizeUpperNoAccents(f?.properties?.Nombre)),
    legendItems: [{ fill: "#E8F0E0", border: "#3F7A36", pattern: "dot", label: "National System of Protected Areas" }],
    style: {
      color: "#3F7A36",
      weight: 1.5,
    ...
    onEachFeature: (f, l, group) => {
      ...
      l.bindPopup(`
        <b>Protected area:</b> ${nombre}<br>
        <b>Category:</b> ${p["Categoría"] ?? "N/A"}<br>
        <b>Type:</b> ${p.Tipo ?? "N/A"}
      `);

      // Etiqueta: [Categoría] + " " + [Nombre], según el catálogo.
      const etiqueta = `${p["Categoría"] ?? ""} ${nombre !== "N/A" ? nombre : ""}`.trim();
  ...
  {
    id: "RedVialEstatal",
    label: "Local transportation network",
    url: "Red_Vial_Estatal_Opt.geojson",
    pane: "pane_tematica",
    legendItems: [{ fill: "#FFFFFF", border: "#FF7F7F", label: "Local transportation network" }],
    style: {
      color: "#FF7F7F",
      weight: 0.8,
      opacity: 0.9,
    ...
    onEachFeature: (f, l) => {
      const p = f.properties || {};
      l.bindPopup(`
        <b>Road:</b> ${p.descripcio ?? "N/A"}<br>
        <b>Type:</b> ${p.typ_desc ?? "N/A"}<br>
        <b>Hierarchy:</b> ${p.hct_desc ?? "N/A"}
      `);
    },
  },

  // ----- Inventario vial (5to envío, GADPA, recortada al área de estudio) -----
  // Sin catálogo de representación propio todavía: se colorea por estado de
  // la superficie de rodadura (estado_sup), la variable central de un
  // inventario vial, con el semáforo bueno/regular/malo de INVENTARIO_VIAL_STYLE.
  {
    id: "InventarioVial",
    label: "Road Inventory (GADPA, Nov 2025)",
    url: "InventarioVial_GADPA_Opt.geojson",
    pane: "pane_tematica",
    legendItems: Object.entries(INVENTARIO_VIAL_STYLE)
      .filter(([key]) => key !== "_default")
      .map(([, info]) => ({ fill: "#FFFFFF", border: info.color, label: `Road surface condition: ${info.label}` })),
    style: (f) => {
      const key = String(f?.properties?.estado_sup ?? "").trim().toUpperCase();
      const info = INVENTARIO_VIAL_STYLE[key] || INVENTARIO_VIAL_STYLE._default;
      return {
        color: info.color,
        weight: 2.2,
        opacity: 0.9,
    ...
    onEachFeature: (f, l) => {
      const p = f.properties || {};
      const lengthKm = typeof p.long_km === "number" ? p.long_km.toFixed(2) : "N/A";
      l.bindPopup(`
        <b>Road code:</b> ${p.cod__via ?? "N/A"}<br>
        <b>Category:</b> ${p.categoria ?? "N/A"}<br>
        <b>Surface type:</b> ${p.tipo_sup ?? "N/A"}<br>
        <b>Surface condition:</b> ${p.estado_sup ?? "N/A"}<br>
        <b>Width (m):</b> ${p.ancho_via ?? "N/A"}<br>
        <b>Length (km):</b> ${lengthKm}<br>
        <b>Connection:</b> ${p.conexion ?? "N/A"}
      `);
    },
  },
  ...
  {
    id: "PobladosIGM250",
    label: "IGM Settlements (1:250,000)",
    ...
    onEachFeature: (f, l, group) => {
      const p = f.properties || {};
      const nombre = p.nam ?? "N/A";
      l.bindPopup(`
        <b>Settlement:</b> ${nombre}<br>
        <b>Type:</b> ${p.na3_desc ?? "N/A"}
      `);
  ...
  // ----- Cobertura y uso del suelo (año 2015) -----
  // id histórico "CoberturaCacao2015" mantenido para no romper referencias;
  // desde el 4to envío usa el shapefile completo cob_2015.shp (convertido a
  // Cobertura_2015_Opt.geojson), con los 4 grupos del Catálogo de
  // representación (2do envío): A (naturales), B (cultivos alimentarios),
  // C (sistemas cacaoteros) y D (otros usos productivos y antrópicos).
  {
    id: "CoberturaCacao2015",
    label: "Land Cover and Land Use (Year 2015)",
    url: "Cobertura_2015_Opt.geojson",
    pane: "pane_tematica",
    // El área cubierta por cob_2015 es específica: sin esto, si el usuario
    // está viendo otra parte del mapa al marcar el checkbox, parece que "no
    // pasó nada" aunque la capa sí se activó.
    fitOnEnable: true,
    ...
    onEachFeature: (f, l) => {
      const p = f.properties || {};
      l.bindPopup(`
        <b>Land cover:</b> ${p.cob_label ?? "N/A"}<br>
        <b>Area:</b> ${p.area_ha ? Number(p.area_ha).toFixed(2) + " ha" : "N/A"}
      `);
    },
  },

  // ----- Mapa oficial de cobertura 2015 (imagen georreferenciada) -----
  // Recorte georreferenciado del mapa OFICIAL "02_1_Cobertura_suelo.pdf"
  // (la columna de leyenda impresa se excluye del recorte; ya tenemos
  // nuestra propia leyenda interactiva). Se conserva como referencia visual
  // del documento oficial, complementaria a la capa vectorial "Cobertura y
  // uso del suelo (año 2015)". El recuadro magenta "Área de estudio" del
  // PDF es un rectángulo perfecto que coincide con Area_Estudio_Opt.geojson,
  // así que sus 4 esquinas dan las coordenadas reales para anclar la imagen
  // sobre el mapa.
  {
    id: "Cobertura2015Raster",
    label: "Official 2015 Land Cover Map (image)",
    type: "image_overlay",
    url: "Cobertura_2015_Mapa.png",
    bounds: [
      [-2.700672857617322, -79.503600587034],
      [-2.500902905031866, -79.37560178739882],
    ],
    fitOnEnable: true,
    legendItems: [{ fill: "#FFFFFF", border: "#6B7280", label: "Official 2015 Land Cover Map (image)" }],
  },
];
...
layersConfig.forEach((cfg) => {
  ...
  if (cfg.id === "CoberturaCacao2015") {
    // Capa con los 4 grupos del catálogo (A-D) en un único dataset
    // (cob_2015.shp): un solo checkbox enciende/apaga todo el dataset; el
    // desglose por grupo (A-D) se ve en la leyenda (cfg.legendGroups), igual
    // que en cualquier otra capa con legendGroups/legendItems.
    div.classList.add("cobertura2015-item");
    const groupsWrap = document.createElement("div");
    groupsWrap.className = "cobertura2015-groups";

    const row = document.createElement("div");
    row.className = "cobertura2015-row";
...
layerListEl.addEventListener("change", async (e) => {
  ...
  // Capas tipo "image_overlay" (mapa oficial recortado del PDF, sin shapefile
  // equivalente): no son GeoJSON, así que se manejan aparte, sin pasar por
  // loadLayerData/fetch+json (fallaría con un PNG) ni por L.geoJSON.
  if (cfg.type === "image_overlay") {
    if (e.target.checked) {
      const overlay = L.imageOverlay(cfg.url, cfg.bounds, { pane: cfg.pane, opacity: cfg.opacity ?? 0.85 });
      const group = L.featureGroup([overlay]);
      group.addTo(map);
      layerStore.set(id, group);
      applyLayerDrawOrder();
      if (cfg.fitOnEnable) {
        try { map.fitBounds(cfg.bounds, { padding: [30, 30] }); } catch (_) {}
      }
    } else {
...
async function fitToStudyArea() {
  try {
    ...
    console.warn("Could not zoom to the study area", err);
  }
...
const CAMBIOS_PRINCIPALES_STYLE = {
  EXPANSION_AGRO: { fill: "#D8BE8B", label: "Agricultural expansion" },
  EXPANSION_PASTO: { fill: "#E4E5AC", label: "Pasture / livestock expansion" },
  EXPANSION_PLANTACION: { fill: "#9BC6BF", label: "Forest plantation expansion" },
  EXPANSION_URBANA: { fill: "#B8B3D3", label: "Urban and infrastructure expansion" },
  GANANCIA_BOSQUE: { fill: "#9FC8A1", label: "Forest recovery / gain" },
  PERDIDA_BOSQUE: { fill: "#D29790", label: "Forest loss" },
};

const PERSISTENCE_BOSQUE_STYLE = { fill: "#B6FDB8", label: "Persistent forest cover" };
const PERSISTENCE_OTROS_STYLE = { fill: "#A8A8A8", label: "Persistence in other uses" };
...
const PERSISTENCE_AGRO_HATCH_STYLE = {
  ...
  label: "Persistence in agricultural use",
};
const PERSISTENCE_PASTO_DOT_STYLE = {
  fill: "#EEE9C9", border: "#734C00", pattern: "dot", svgFill: "url(#dotPasto)",
  label: "Persistence in pasture and livestock",
};

const RECONFIG_STYLE_BY_LAYER = {
  [ID_1990_2000]: {
    "AGRO → NATURAL_SECUNDARIO": { fill: "#4C9C74", label: "Reconfiguration: agricultural → secondary vegetation" },
    "PASTO_GANADERIA → NATURAL_SECUNDARIO": { fill: "#7CB43F", label: "Reconfiguration: pasture/livestock → secondary vegetation" },
    "AGRO → AGUA": { fill: "#3485C4", label: "Reconfiguration: agricultural → hydrographic area" },
  },
  [ID_2000_2020]: {
    "PASTO_GANADERIA → NATURAL_SECUNDARIO": { fill: "#80B143", label: "Reconfiguration: pasture/livestock → secondary vegetation" },
    "PLANTACION → NATURAL_SECUNDARIO": { fill: "#C5E9FF", label: "Reconfiguration: forest plantation → secondary vegetation" },
    "PASTO_GANADERIA → AGUA": { fill: "#33ACDA", label: "Reconfiguration: pasture/livestock → hydrographic area" },
    "NO_DATA → NATURAL_SECUNDARIO": { fill: "#8CA176", label: "No prior information → secondary vegetation" },
  },
  [ID_2020_2022]: {
    "NATURAL_SECUNDARIO → AGUA": { fill: "#4892C0", label: "Reconfiguration: secondary vegetation → hydrographic area" },
    "AGRO → NATURAL_SECUNDARIO": { fill: "#509B6D", label: "Reconfiguration: agricultural → secondary vegetation" },
    "PLANTACION → NATURAL_SECUNDARIO": { fill: "#1E836D", label: "Reconfiguration: forest plantation → secondary vegetation" },
    "NO_DATA → NATURAL_SECUNDARIO": { fill: "#909F74", label: "No prior information → secondary vegetation" },
  },
};
...
const CACAO_TRANS_STYLE = {
  "FOREST → AGRO": {
    ...
    label: "Conversion of forest to agricultural and cacao uses",
  },
  "PASTO_GANADERIA → AGRO": {
    fill: "#FFFFFF", border: "#DE394D",
    label: "Conversion of pasture and livestock to agricultural and cacao uses",
  },
  "AGUA → AGRO": {
    fill: "#A8ECFD", border: "#2C7FB8", pattern: "dot", svgFill: "url(#dotAgua)",
    label: "Water area reclassified to agricultural use associated with cacao",
  },
  "NO_DATA → AGRO": {
    fill: "#FFFFFF", border: "#6B7280",
    label: "Conversion of land cover with no prior information to agricultural and cacao use",
  },
};
...
function getTransitionFeatureStyle(properties, categoryField, layerId) {
  ...
  if (layerId === ID_CACAO) {
    ...
    return { fill: "#FFFFFF", border: "#6B7280", label: "Transition to cacao (unclassified)" };
  }
  ...
  if (grp.startsWith("RECONFIGURACION")) {
    ...
    return { fill: "#8A7FAE", label: "Internal reconfiguration (other combination)" };
  }
  ...
  return { fill: transitionState.categoryColorMap.get(key), label: grpRaw ? String(grpRaw) : "No information" };
}
...
const LEGEND_DEFS = {
  [ID_1990_2000]: {
    title: "LEGEND 1990–2000",
    sections: [
      { title: "Main land cover changes", items: Object.values(CAMBIOS_PRINCIPALES_STYLE) },
      { title: "Persistent land cover", items: [PERSISTENCE_BOSQUE_STYLE, PERSISTENCE_OTROS_STYLE] },
      { title: "Internal reconfiguration", items: Object.values(RECONFIG_STYLE_BY_LAYER[ID_1990_2000]) },
    ],
  },
  [ID_2000_2020]: {
    title: "LEGEND 2000–2020",
    sections: [
      { title: "Main land cover changes", items: Object.values(CAMBIOS_PRINCIPALES_STYLE) },
      { title: "Persistent land cover", items: [PERSISTENCE_BOSQUE_STYLE, PERSISTENCE_OTROS_STYLE] },
      { title: "Internal reconfiguration", items: Object.values(RECONFIG_STYLE_BY_LAYER[ID_2000_2020]) },
      {
        title: "Transition towards cacao (2015 information)",
  ...
  [ID_2020_2022]: {
    title: "LEGEND 2020–2022",
    sections: [
      { title: "Main land cover changes", items: Object.values(CAMBIOS_PRINCIPALES_STYLE) },
      {
        title: "Persistent land cover",
        items: [PERSISTENCE_BOSQUE_STYLE, PERSISTENCE_AGRO_HATCH_STYLE, PERSISTENCE_PASTO_DOT_STYLE, PERSISTENCE_OTROS_STYLE],
      },
      { title: "Internal reconfiguration", items: Object.values(RECONFIG_STYLE_BY_LAYER[ID_2020_2022]) },
    ],
  },
};
...
function buildTransitionPopup(feature, layerCfg) {
  ...
  return `
    ...
    <b>Period:</b> ${htmlEscape(layerCfg.timeLabel || "N/A")}<br>
    <b>Category:</b> ${htmlEscape(info.label)}<br>
    ${rows}
  `;
}
...
async function showTransitionIndex(index) {
  ...
  if (els.label) {
    els.label.textContent = `${cfg.timeLabel || cfg.label} · ${cfg.featureCount ?? ""} features`;
  }
...
function renderTransitionLayerList() {
  ...
  els.list.innerHTML = transitionState.layers.map((cfg, idx) => {
    ...
    const labelText = isCacao
      ? "Transition to cacao (automatic, over 2000–2020)"
...
async function initTransitionWidget() {
  ...
  try {
    ...
    if (!transitionState.timelineLayers.length) {
      els.status.textContent = "No transition layers registered.";
      return;
    }

    const hasCacao = transitionState.layers.some((l) => l.id === ID_CACAO);
    els.status.textContent = `${transitionState.timelineLayers.length} transition scenarios available`
      + (hasCacao ? " (cacao overlay is added automatically over 2000–2020)" : "");
    ...
    els.status.textContent = "Transition layers could not be loaded. Run convertir_transicion_geojson.py first";
    console.warn("Multitemporal transition:", err);
  }
}
